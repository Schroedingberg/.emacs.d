...
(eval-when-compile
  (unless (and (fboundp 'dolist)
               ...
               (fboundp 'pop))
    (require 'cl))) ;; for macros in past emacs
...
;; Emacs usually guesses utf-8 correctly itself, so don't think need file
;; coding setups.
;;
;; ;;;###autoload
;; (when (memq 'utf-8 (coding-system-list))
;;   (modify-coding-system-alist 'file "/A[0-9]\\{6,7\\}\\.internal\\.txt\\'")
;;   (modify-coding-system-alist 'file "/OEIS/names\\'"))
...
(defgroup oeis nil
  ...
  :prefix "oeis-"
  ;; FIXME: Is group `external' good?  It's meant for external programs.
...
\"data\" and \"short\" are the same when matching numbers.  When
matching text \"short\" shows the relevant paragraph."

  ;; sample URLs showing the difference between short and data
  ;; http://oeis.org/search?fmt=short&q=bloodshed
  ;; http://oeis.org/search?fmt=data&q=bloodshed
...
Return a filename with directory if found, or nil if not."
...
(eval-when-compile
  (put 'oeis-anum-at-point 'side-effect-free t))
...
TYPE is one of the following symbols.  nil or omitted means `html'.
  ...
  (cond ((eq type 'b-file)
         (concat "http://oeis.org/" anum "/b" (substring anum 1) ".txt"))
        ((eq type 'a-file)
         (concat "http://oeis.org/" anum "/a" (substring anum 1) ".txt"))
        ((eq type 'internal-text)
         (concat "http://oeis.org/search?q=id:" anum "&fmt=text"))
        ((eq type 'internal-html)
         (concat "http://oeis.org/" anum "/internal"))
        (t
         (concat "http://oeis.org/" anum))))
(eval-when-compile
...
(defadvice ffap-string-at-point (around oeis disable)
  ...
  ;; Defang our code here with an fboundp.
...
http://oeis.org/A123456/a123456.txt."
...
See `oeis-search-format' to choose short or long results."
  ...
  (browse-url (concat "http://oeis.org/search?fmt=" oeis-search-format
                      "&q=" str)))
...
The search help page is ")
        (insert-button "http://oeis.org/ol.html"
                       'face 'link
...
and superseeker help    ")
        (insert-button "http://oeis.org/superhelp.txt"
                       'face 'link
                       'help-echo (concat "mouse-2, RET: Follow this link")
                       'action 'oeis-button-browse-url
                       'follow-link t)
...
(defun oeis-superseeker-check-time ()
  "An internal part of oeis.el.
Check the OEIS superseeker lookup time limit has passed..
...
to continue anyway.  Or delete the file."
  ...
  (when oeis-superseeker-limit-seconds
    ...
    (let ((seconds
      ...
      (when (and seconds
                 (> seconds 0))
        (yes-or-no-p (format "Superseeker limit one per hour, next in %d minutes.  Continue? "
                             (/ (+ seconds 59) 60)))))))
...
;; LocalWords: foo grep minibuffer superseeks
